...
# Define paths
$scriptPath = Join-Path -Path $PSScriptRoot -ChildPath "Account-Status.ps1"
$exePath = Join-Path -Path $PSScriptRoot -ChildPath "AccountStatusChecker.exe"
...
$params = @{
    ...
    OutputFile = $exePath
    NoConsole = $true  # Hide console for clean GUI-only mode
